            ...
            {{ csrf_field() }}
              <div class="box-body">
                    ...
                    <div class="form-group @if(!empty($errors->first('listname'))) has-error @endif">
                        ...
                        <input type="text" name="eventName"  class="form-control" placeholder=""  value="{{$eventInfo->event_name}}">
                    ...
                    <div class="form-group @if(!empty($errors->first('event_date'))) has-error @endif">
                        <label for="exampleInputFile">{{trans('event.item.event_date')}}</label>
                                <div class="input-group date">
                                    ...
                                    <input type="text" class="form-control datepicker" name="event_date"  value="{{$eventInfo->event_date}}" id="event_date">
                                </div>
                    </div>
                    ...
                    <div class="form-group @if(!empty($errors->first('event_place'))) has-error @endif">
                        ...
                        <input type="text" name="event_place"  class="form-control" placeholder=""  value="{{$eventInfo->event_place}}">
                        <span class="help-block">{{$errors->first('event_place')}}</span>
                    </div>

                    <!-- チャット期間 -->
                    <div class="form-group @if(!empty($errors->first('send_date'))) has-error @endif">
                        <label for="exampleInputFile">{{trans('event.item.chat_period')}}</label>
                               <div class="input-group date">
                                <div class="input-group-addon">
                                    <i class="fa fa-calendar"></i>
                                </div>
                                <input type="text" class="form-control datepicker" name="start_date"  id="start_date"
                                    value="{{date(Lang::get('website.date_format_home'), strtotime($eventInfo->event_start))}} to {{date(Lang::get('website.date_format_home'), strtotime($eventInfo->event_end))}}" >
                            </div>
                    </div>
                    ...
                    <div class="form-group @if(!empty($errors->first('notes'))) has-error @endif">
                        <label for="exampleInputFile">{{trans('event.item.target')}}{{$eventInfo->target_id}}</label>
                        <select class="form-control select2" style="width: 100%;" name="target">
                            @foreach ($Lists as $row)
                            <option value='{{$row->id}}'  @if($row->status ==0)  disabled="disabled" @endif @if($row->id ==$eventInfo->target_id) selected @endif>{{$row->name}}</option>
                            @endforeach
                        </select>
                    </div><!-- /.form group -->
